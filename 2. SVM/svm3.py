...
svm3 = svm.SVC(kernel='poly', degree=3, C=C).fit(X_train, y)
y_hat3 = svm3.predict(space3D)
...
ax.scatter(space3D[:, 0], space3D[:, 1], space3D[:, 2], c=y_hat3, cmap=cmap, marker='s', s=100, alpha=.05)

plt.title('SVM Classification (Polynomial Kernel) | Degree 3 & Regularization: C = 1', fontweight='bold')
plt.legend()
plt.show()