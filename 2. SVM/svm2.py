...
svm2 = svm.SVC(kernel='rbf', gamma=0.2, C=C).fit(X_train, y)        # rbf: the (Gaussian) radial basis function Kernel
y_hat2 = svm2.predict(space3D)
...
ax.scatter(space3D[:, 0], space3D[:, 1], space3D[:, 2], c=y_hat2, cmap=cmap, marker='s', s=100, alpha=.05)

plt.title('SVM Classification (RBF Kernel) | Regularization: C = 1', fontweight='bold')
plt.legend()
plt.show()